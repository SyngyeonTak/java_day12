...
public class ClassInfo {//extends Object
	...
	public ClassInfo() {
		...
		Method[] methods = myClass.getMethods(); //현재 인스턴스가 보유한 메서드를 반환
		
		for (int i = 0; i < methods.length; i++) {
			System.out.println("현재 객체가 보유한 메서드는 "+methods[i]);			
		}
		//Class 클래스를 이용하면, 해당 클래스의 환경정보 등도 접근할 수 있다.. 따라서 우리는
		//classpath를 조사해서 우리가 사용하는 res라는 패키지의 경로를 조사할 것임
		
		//아래의 메서드를 이용하면, 클래스 패스를 기준으로 자원에 접근할 수 있다...
		URL url = myClass.getClassLoader().getResource("res/whatsapp.png");//res가 루트이다...(클래스가 아닌 일반 자원은 /로 표현)
